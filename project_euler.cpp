...
void problem_13()
{
    ...
    // sum up all of the given numbers using bigints
    vectored_bigint val;
    for(int i = 0; i < 100; i++)
        val = val + vectored_bigint(problem_13_data[i]);

    string result = val.to_string();
    string result_first_10_digits = result.substr(0, 10);

    cout << "first ten digits of the sum of one-hundred 50-digit numbers: " << result_first_10_digits << endl;
}
